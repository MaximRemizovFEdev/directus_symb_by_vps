...
- ветка `dev-v1`;
- SSH-ключ `%USERPROFILE%\.ssh\symbolika_vps`.
...
Другой SSH-ключ можно передать параметром `-IdentityFile`.
...
3. перед изменением кода создаёт полный комплект восстановления в `/opt/symbolika/backups`: проверенный PostgreSQL dump, архив `uploads`, закрытую копию `.env` и файл с номером коммита;
