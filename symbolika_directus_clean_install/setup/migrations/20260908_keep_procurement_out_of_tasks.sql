...
-- Archive unfinished generated tasks, retain completed history, and remove all
-- live links so later procurement changes cannot affect the task tracker.
...
SET task_order_id = NULL,
    task_payment_id = NULL,
    task_pickup_id = NULL,
    date_updated = now()
WHERE task_order_id IS NOT NULL
   OR task_payment_id IS NOT NULL
   OR task_pickup_id IS NOT NULL;

UPDATE procurement_batches
SET task_order_id = NULL,
    management_task_id = NULL,
    task_payment_id = NULL,
    task_pickup_id = NULL,
    date_updated = now()
WHERE task_order_id IS NOT NULL
   OR management_task_id IS NOT NULL
   OR task_payment_id IS NOT NULL
   OR task_pickup_id IS NOT NULL;
