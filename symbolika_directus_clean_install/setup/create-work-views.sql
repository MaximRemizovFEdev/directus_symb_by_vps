...
-- Procurement is managed entirely in its own workspace. Do not duplicate its
-- order, payment or pickup workflow in the general task tracker.
DROP TRIGGER IF EXISTS procurement_purchase_task_sync ON procurement_requests;
DROP TRIGGER IF EXISTS procurement_status_sync_from_task ON symbolika_tasks;
...
BEGIN
  ...
  IF NEW.status = 'received' THEN
    IF NEW.received_at IS NULL THEN
      UPDATE procurement_requests
      SET received_at = now(), date_updated = now()
      WHERE id = NEW.id;
    END IF;
    UPDATE orders_items
    SET blank_ordered = true
