...
Сводка, события, проблемы, поиск, все заказы, сроки, собственные заказы, расчеты, архивы и офис. Заказ содержит позиции; позиция является главным объектом производства, ТЗ, макета, себестоимости и маршрутизации.
...
Очереди производства и шелкографии, общий архив, этикетки, склад и расходка. Пользователь участка видит только свои позиции.
...
Сверки, клиентские операции и подарочные сертификаты.

### Клиенты

Клиенты и компании, карточки, балансы, контакты и передача менеджера.
...
Полная настройка: [NOTIFICATIONS_SETUP.md](NOTIFICATIONS_SETUP.md). Клиентские каналы: [CUSTOMER_NOTIFICATIONS_SETUP.md](CUSTOMER_NOTIFICATIONS_SETUP.md).
...
Перемещение по рабочим папкам не выполняет IMAP MOVE. Полная настройка: [MAIL_CLIENT_SETUP.md](MAIL_CLIENT_SETUP.md).
...
Подробности: [YANDEX_DISK_SETUP.md](YANDEX_DISK_SETUP.md). Если описание структуры имен в старом документе расходится с кодом, источником истины является текущий endpoint.
