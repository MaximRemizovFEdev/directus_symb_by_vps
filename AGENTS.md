...
- `symbolika_directus_clean_install/extensions/symbolika-tbank/` — защищённое выставление счетов Т‑Банка из карточки заказа; банковский токен хранится только в серверном окружении.
...
- Позиции категории «Разработка дизайна» видны в заказе, но не участвуют в офисной выдаче и всегда имеют пустой офисный статус.
...
- Изменение оклада или процента сотрудника действует с текущего расчётного месяца; прошлые месяцы используют историю `employee_compensation_rates` и не пересчитываются новой ставкой.
